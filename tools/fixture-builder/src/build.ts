...
export async function buildFixture(): Promise<BuildResult> {
  ...
  return {
    labels: {
      ...
      // Generated by the same hand that wrote the detectors, so it is the set
      // thresholds may be chosen against, never the one they are reported on.
      split: "tuning",
      notes:
...
async function buildDetachment(page: PageNode, card: ComponentNode, recorder: Recorder): Promise<void> {
  ...
  // Structurally changed, so its shape no longer matches and only the name is
  // left to carry it. It is labelled all the same: the builder detached this
  // frame and knows it did, and a label is ground truth rather than a guess at
  // what the detector will manage. Leaving it out would buy a better recall
  // number by declining to ask the hard question.
  await recorder.step("Detached edited", () => {
    ...
    recorder.expect({
      ...
      field: "structure",
      category: "detachment",
      why: "an instance was detached and then restructured, so only its name is left to recognise it by",
    })
    recorder.expect({
      page: USAGE_PAGE,
      path: "Detached edited / Card",
